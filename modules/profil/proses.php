...
// fungsi untuk pengecekan status login profil 
// jika profil belum login, alihkan ke halaman login dan tampilkan pesan = 1
if (empty($_SESSION['username']) && empty($_SESSION['password'])) {
	echo "<meta http-equiv='refresh' content='0; url=index.php?alert=1'>";
}
// jika profil sudah login, maka jalankan perintah untuk insert dan update
else {
	// update data
	if ($_GET['act'] == 'update') {
		if (isset($_POST['simpan'])) {
			...
			$id_profil            = mysqli_real_escape_string($mysqli, trim($_POST['id_profil']));
				...
				$path_file          = "../../assets/img/user/" . $nama_file;
				...
				if (empty($_FILES['foto']['name'])) {
					...
					$query = mysqli_query($mysqli, "UPDATE profil SET
                    												id_profil  		= '$id_profil ',
																				nama 			= '$nama',
				...
				if (in_array($extension, $allowed_extensions)) {
					...
					if ($ukuran_file <= 1000000) { // Cek apakah ukuran file yang diupload kurang dari sama dengan 1MB
						...
						header(
							"location: ../../main.php?module=profil&alert=2"
						);
					}
				} else {
